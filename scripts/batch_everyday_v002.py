#!/usr/bin/env python3
"""
Batch generate ~200 everyday lifestyle photos using LoRA v002.
Diverse scenes, natural/imperfect lighting, authentic feel.

Usage:
    python scripts/batch_everyday_v002.py \
        --comfyui-url https://xve3vkwv9je132-8188.proxy.runpod.net \
        --count-per-prompt 8
"""
...
LORA_NAME = "lora_chara_v002.safetensors"
LORA_STRENGTH = 0.8
FORMAT = "4:5"
OUTPUT_DIR = Path("./explore_output/batch_everyday_v002")


def main():
    import argparse
    parser = argparse.ArgumentParser(description="Batch everyday photos with LoRA v002")
    parser.add_argument("--comfyui-url", type=str, required=True)
    parser.add_argument("--count-per-prompt", type=int, default=4,
                        help="Seeds per prompt (default: 4, total = prompts x count)")
